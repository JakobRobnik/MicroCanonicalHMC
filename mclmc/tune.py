...
def tune12(dynamics, d, adjust, 
           ...
           acc_prob_wanted = 0.7, t0 = 10, gamma_dual= 0.05, kappa= 0.75): # these parameters will have no effect if adjust = False
    ...
    def predictor(dyn_old, hyp, adaptive_state):
        ...
        hyp_new = Hyperparameters(hyp.L, eps, hyp.sigma, hyp.Ld)
        
        return dyn, hyp_new, hyp_new, (W, F, eps_max), success


    def dual_averaging(dyn, hyp, adap):
        
        dyn, energy_change = dynamics(dyn, hyp)
        
        acc = jnp.clip(jnp.exp(-energy_change), 0., 1.)
        adap = dual_avg_update(acc, adap, acc_prob_wanted, t0, gamma_dual, kappa)
        
        hyp = Hyperparameters(hyp.L, jnp.exp(adap[0]), hyp.sigma, hyp.Ld)
        hyp_final = Hyperparameters(hyp.L, jnp.exp(adap[1]), hyp.sigma, hyp.Ld)
        
        return dyn, hyp, hyp_final, adap, True
    ...
    def func(_dyn, _hyp, num_steps):
        ...
        return dyn, Hyperparameters(L, hyp.eps, sigma, hyp.Ld)
...
def tune3(step, frac, Lfactor):
    ...
    def func(dyn, hyp, num_steps):
        ...
        return dyn, Hyperparameters(Lnew, hyp.eps, hyp.sigma, hyp.Ld)
...
def dual_avg_update(acc_prob, adap, acc_prob_wanted, t0, gamma, kappa):
    """taken from numpyro, see documentation there"""

    g = acc_prob_wanted - acc_prob
    
    x_t, x_avg, g_avg, t = adap
