...
def bisection(f, a, b, tol=1e-3, max_iter=100):
    ...
    a, b, midpoint, iter_count = jax.lax.cond(
        f(b) <= 0,
        lambda _: (b, b, b, 0),
...
class Sampler:
    ...
    def update_momentum(self, eps, g, u):
        ...
        nonzero = (
            g_norm > 1e-13
        )  # if g_norm is zero (we are at the MAP solution) we also want to set e to zero and the function will return u
        ...
        zeta = jnp.exp(-delta)
        uu = (
            e * ((1 - zeta) * (1 + zeta + ue * (1 - zeta)))[:, None]
            + 2 * zeta[:, None] * u
        )
    ...
    def dynamics(self, x, u, g, random_key, L, eps, T):
        ...
        # Hamiltonian step
        xx, uu, ll, gg, kinetic_change, key = self.hamiltonian_dynamics(
            x, u, g, random_key, eps, T
        )
    ...
    def sample_temp_level(
        self, num_steps, tune_steps, x0, u0, l0, g0, key0, L0, eps0, T
    ):
        def step(state, tune):
            ...
            ### eps tuning ###
            de = (
                jnp.square(kinetic_change + ll - l) / self.Target.d
            )  # square energy error per dimension
    ...
    def sample(
        ...
        def temp_level(state):
            ...
            x, u, l, g, key, L, eps, T = resample_particles(
                logw, x, u, l, g, key, L, eps, T
            )

            x, u, l, g, key, L, eps = self.sample_temp_level(
                steps_at_each_temp, tune_steps, x, u, l, g, key, L, eps, T
            )
