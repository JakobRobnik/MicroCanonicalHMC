...
class Sampler:
    ...
    def __init__(
        ...
        self.frac_tune1 = (
            frac_tune1  # num_samples * frac_tune1 steps will be used to autotune eps
        )
        ...
        self.gamma = (neff - 1.0) / (
            neff + 1.0
        )  # forgeting factor in the adaptive step
    ...
    def dynamics_adaptive(self, state, L, sigma):
        ...
        # step updating
        success, xx, uu, ll, gg, eps_max, kinetic_change = self.nan_reject(
    ...
    def single_chain_sample(self, num_steps, x_initial, random_key, output, thinning):
        ...
        L, eps = (
            self.L,
            self.eps,
        )  # the initial values, given at the class initialization (or set to the default values)

        sigma = jnp.ones(
            self.Target.d
        )  # jnp.ones(self.Target.d)  # no diagonal preconditioning
        ...
        if self.frac_tune1 + self.frac_tune2 + self.frac_tune3 != 0.0:
            ...
            L, eps, sigma, x, u, l, g, key = self.tune12(
                x, u, l, g, key, L, eps, sigma, steps1, steps2
            )  # the cheap tuning (100 steps)
            if (
                self.frac_tune3 != 0
            ):  # if we want to further improve L tuning we go to the second stage (which is a bit slower)
        ...
        if output == "normal" or output == "detailed":
            X, _, E = self.sample_normal(
                num_steps, x, u, l, g, key, L, eps, sigma, thinning
            )
            if output == "detailed":
                return X, E, L, eps
            else:
            ...
            raise ValueError(
                "output = " + output + "is not a valid argument for the Sampler.sample"
            )
    ...
    def sample_normal(self, num_steps, x, u, l, g, random_key, L, eps, sigma, thinning):
        ...
        def step(state, useless):
            x, u, l, g, key = state
            xx, uu, ll, gg, kinetic_change, key = self.dynamics(
                x, u, g, key, L, eps, sigma
            )
        ...
        if thinning == 1:
            return jax.lax.scan(
                step, init=(x, u, l, g, random_key), xs=None, length=num_steps
            )[1]

        else:
            return self.sample_thinning(
                num_steps, x, u, l, g, random_key, L, eps, sigma, thinning
            )

    def sample_thinning(
        self, num_steps, x, u, l, g, random_key, L, eps, sigma, thinning
    ):
        """Stores transform(x) for each step."""
        ...
        def step(state, useless):
            ...
            state = jax.lax.scan(substep, init=state, xs=None, length=thinning)[
                0
            ]  # do 'thinning' steps without saving

            return state, (
                self.Target.transform(state[0]),
                state[2],
                state[4],
            )  # save one sample
        ...
        return jax.lax.scan(
            step,
            init=(x, u, l, g, 0.0, random_key),
            xs=None,
            length=num_steps // thinning,
        )[1]
    ...
    def sample_expectation(self, num_steps, x, u, l, g, random_key, L, eps, sigma):
        ...
        return jax.lax.scan(
            step, init=(x, u, g, random_key), xs=None, length=num_steps
        )[0][1][1]
    ...
    def sample_ess(self, num_steps, x, u, l, g, random_key, L, eps, sigma):
        ...
        def step(state_track, useless):
            ...
            W += 1
            bias_d = (
                jnp.square(F2 - self.Target.second_moments)
        ...
        _, b = jax.lax.scan(
            step,
            init=(
                (x, u, l, g, 0.0, random_key),
                (1, jnp.square(self.Target.transform(x))),
            ),
            xs=None,
            length=num_steps,
        )
    ...
    def tune12(
        self, x, u, l, g, random_key, L_given, eps, sigma_given, num_steps1, num_steps2
    ):
        ...
        def step(state, outer_weight):
            ...
            x, u, l, g, E, Feps, Weps, eps_max, key, eps = self.dynamics_adaptive(
                state[0], L, sigma
            )
            W, F1, F2 = state[1]
            w = outer_weight * eps
    ...
    def tune3(self, x, u, l, g, random_key, L, eps, sigma, num_steps):
        ...
        X, xx, uu, ll, gg, key = self.sample_full(
            num_steps, x, u, l, g, random_key, L, eps, sigma
        )
        ESS = ess_corr(X)
    ...
    def sample_full(self, num_steps, x, u, l, g, random_key, L, eps, sigma):
        ...
        state, track = jax.lax.scan(
            step, init=(x, u, l, g, 0.0, random_key), xs=None, length=num_steps
        )
    ...
    def bias_plot(self, results):
        ...
        return (
            100.0 / (find_crossing(bsq, 0.01) * self.grad_evals_per_step)
        ) * cutoff_reached
...
def find_crossing(array, cutoff):
    ...
    def step(carry, element):
        ...
        never_been_below = (
            carry[1] * above_threshold
        )  # 1 if (array[i] > cutoff for all i < current index) else 0
...
def point_reduction(num_points, reduction_factor):
    ...
    indexes = np.concatenate(
        (
            np.arange(1, 1 + num_points // reduction_factor, dtype=int),
            np.arange(
                1 + num_points // reduction_factor,
                num_points,
                reduction_factor,
                dtype=int,
            ),
