...
def sample_nuts(target, mchmc_target, target_params, num_samples, num_warmup, thinning, random_key=None, progress_bar= False, trajectory_length = None):
    """if trajectory_length == None, runs NUTS, else, runs HMC"""
    ...
    # setup
    if trajectory_length == None:
        nuts_setup = NUTS(target, adapt_step_size=True, adapt_mass_matrix=True, dense_mass= False)  # originally: nuts_kernel
    else:
        nuts_setup = HMC(target, num_steps = trajectory_length, adapt_step_size=True, adapt_mass_matrix=True, dense_mass=False)
    ...
    if target_params == None:        
        ...
        sampler.warmup(key_warmup, *target_params, init_params=x0, extra_fields=['num_steps'], collect_warmup=True)
        warmup_steps = np.sum(np.array(sampler.get_extra_fields()['num_steps'], dtype=int))
