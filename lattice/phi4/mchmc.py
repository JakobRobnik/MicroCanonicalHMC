...
def load_data(index):
    ...
    integrator = 'MN'
    lam = theory.unreduce_lam(theory.reduced_lam, side)
...
#     target = theory.Theory(side, lamb)
...
#     chi_reduced = theory.reduce_chi(chi, side)
...
#     #draw from the prior
#     x = jax.vmap(theory.Theory(side, lam[15]).prior_draw)(jax.random.split(jax.random.PRNGKey(42), chains))
...
#     np.save(dir+'/data/mchmc/ground_truth/chi/L'+str(side)+'.npy', data)
...
def ground_truth():
    ...
    np.save(dir+'/data/mchmc/ground_truth/chi/LL'+str(side)+'.npy', data)
...
def tuning_free():
    ...
    #draw from the prior
    target = theory.Theory(side, lam[15])
    ...
    df.to_csv(dir + '/data/mchmc/ess/psd/autotune/L' + str(side) + '.csv', index =False)
...
def grid_search():
    ...
    np.save(dir + '/data/mchmc/ess/psd/grid_search/L' + str(side) + '.npy', data)
    
    
def large_lattice():
    side = 2**10
    lamb = theory.unreduce_lam(theory.reduced_lam, side)[-1]
    ...
    chi = theory.reduce_chi(jax.vmap(target.susceptibility2_full, (0, None))(phibar, side))
    
    np.save(dir + '/data/mchmc/large_lattice/L' + str(side) + '.npy', chi)
