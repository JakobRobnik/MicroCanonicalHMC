...
def run_unadjusted_mclmc_no_tuning(initial_state, integrator_type, step_size, L, sqrt_diag_cov):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        return MCLMCAdaptationState(L=L, step_size=step_size, sqrt_diag_cov=sqrt_diag_cov),  calls_per_integrator_step(integrator_type), 1., expectations
...
def run_adjusted_mclmc_no_tuning(initial_state, integrator_type, step_size, L, sqrt_diag_cov, L_proposal_factor=jnp.inf):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        jax.debug.print("trace_at_every_step {x}", x=info.acceptance_rate.mean())

        return MCLMCAdaptationState(L=L, step_size=step_size, sqrt_diag_cov=sqrt_diag_cov), num_steps_per_traj * calls_per_integrator_step(integrator_type), info.acceptance_rate.mean(), expectations

    return s

def run_mclmc(integrator_type, preconditioning, frac_tune3=0.1):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        return run_unadjusted_mclmc_no_tuning(blackjax_state_after_tuning, integrator_type, blackjax_mclmc_sampler_params.step_size, blackjax_mclmc_sampler_params.L, blackjax_mclmc_sampler_params.sqrt_diag_cov)(model, num_steps, initial_position, run_key)

    return s

def run_adjusted_mclmc(integrator_type, preconditioning, frac_tune1=0.1, frac_tune2=0.1, frac_tune3=0.1, L_proposal_factor=jnp.inf, target_acc_rate=None):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        if target_acc_rate is None:
            new_target_acc_rate = target_acceptance_rate_of_order[integrator_order(integrator_type)]
        ...
        return run_adjusted_mclmc_no_tuning(blackjax_state_after_tuning, integrator_type, step_size, L, blackjax_mclmc_sampler_params.sqrt_diag_cov, L_proposal_factor)(model, num_steps, initial_position, run_key)
...
def run_nuts(integrator_type, preconditioning):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        #     num_steps=num_steps,
        #     transform=lambda state,info: (model.transform(state.position), info),
        #     progress_bar=True
        # )
        jax.debug.print("{x} num_steps", x=info.num_integration_steps.mean())

        return params, info.num_integration_steps.mean() * calls_per_integrator_step(integrator_type), info.acceptance_rate.mean(), expectations
