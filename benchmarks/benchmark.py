    ...
    for model in models:
        ...
        for integrator_type in integrators:
            ...
            if grid:
                ...
                unadjusted_kernel = (
                    lambda sqrt_diag_cov: blackjax.mcmc.mclmc.build_kernel(
                        logdensity_fn=model.logdensity_fn,
                        integrator=integrator,
                        sqrt_diag_cov=sqrt_diag_cov,
                    )
                ...
                if do_grid_search_for_adjusted:
                    ...
                    ess, ess_avg, ess_corr, params, acceptance_rate, _ = (
                        benchmark_chains(
                            model,
                            run_adjusted_mclmc_no_tuning(
                                integrator_type=integrator_type,
                ...
                if do_grid_search_for_unadjusted:
                    ...
                    def func_unadjusted(L, step_size):
                        ...
                        r = benchmark_chains(
                            ...
                            sampler=run_unadjusted_mclmc_no_tuning(
                                integrator_type=integrator_type,
                                initial_state=unadjusted_blackjax_state_after_tuning,
                    ...
                    jax.debug.print(
                        "Initial grid search params {x}",
                        x=(
                            blackjax_unadjusted_mclmc_sampler_params.L,
                            blackjax_unadjusted_mclmc_sampler_params.step_size,
                        ),
                    )

                    out, edge = grid_search(
                        func=func_unadjusted,
                        x=blackjax_unadjusted_mclmc_sampler_params.L,
                        y=blackjax_unadjusted_mclmc_sampler_params.step_size,
                        delta_x=blackjax_unadjusted_mclmc_sampler_params.L - 0.2,
                        delta_y=blackjax_unadjusted_mclmc_sampler_params.step_size
                        - 0.2,
                        size_grid=5,
                        num_iter=4,
                    )
            ...
            for preconditioning in [False]:
                ...
                if True:
                    ess, ess_avg, ess_corr, params, _, step_size_over_da = (
                        benchmark_chains(
                            model,
                            partial(
                                run_mclmc,
                ...
                ess, ess_avg, ess_corr, _, acceptance_rate, _ = benchmark_chains(
                    model,
                    partial(
                        run_nuts,
                        integrator_type=integrator_type,
...
def run_benchmarks_simple():
    ...
    for i in range(1):
        ...
        for preconditioning in [False]:
            ...
            print(
                f"Effective Sample Size (ESS) of {model.name} with preconditioning set to {preconditioning} is {ess_avg}"
            )
...
def test_benchmarking():
    ...
    ess, ess_avg, ess_corr, _, acceptance_rate = benchmark_chains(
        model,
        run_adjusted_mclmc(integrator_type=integrator_type, preconditioning=False, frac_tune3=0.0),
        key1,
        n=num_steps,
        batch=num_chains,
    )
