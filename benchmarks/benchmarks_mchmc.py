...
class IllConditionedGaussian:
    ...
    def __init__(self, d, condition_number, numpy_seed=None, prior="prior"):
        ...
        if prior == "map":
            ...
            self.prior_draw = lambda key: self.R @ (
                jax.random.normal(key, shape=(self.d,)) * jnp.sqrt(eigs)
            )

        else:  # N(0, sigma_true_max)
            self.prior_draw = lambda key: jax.random.normal(
                key, shape=(self.d,)
...
class IllConditionedGaussianGamma:
    ...
    def __init__(self, prior="prior"):
        ...
        eigs = np.sort(
            rng.gamma(shape=0.5, scale=1.0, size=self.d)
...
class Banana:
    ...
    def __init__(self, prior="map"):
        ...
        if prior == "map":
            ...
            self.prior_draw = (
                lambda key: jax.random.normal(key, shape=(self.d,))
                * jnp.array([10.0, 5.0])
                * 2
            )
        else:
    ...
    def plott(self):
        xmin, xmax = -20.0, 20.0
        ymin, ymax = -10.0, 10.0
        X, Y, Z = get_contour_plot(
            self, jnp.linspace(xmin, xmax, 100), jnp.linspace(ymin, ymax, 100)
        )
...
class Cauchy:
    ...
    def __init__(self, d):
        ...
        self.nlogp = lambda x: jnp.sum(jnp.log(1.0 + jnp.square(x)))
        self.grad_nlogp = jax.value_and_grad(self.nlogp)
        self.transform = lambda x: x
...
class HardConvex:
    ...
    def prior_draw(self, key):
        ...
        scale = jnp.concatenate(
            (jnp.ones(self.d - 1), jnp.ones(1) * jnp.sqrt(2.0 * self.kappa / 3.0))
        )
...
class BiModal:
    ...
    def __init__(self, d=50, mu=8.0, sigma1=1.0, sigma2=1.0, f=0.2):
        ...
        Vx1, Vx2 = (
            2 * sigma1**4,
            2 * sigma2**4 + 4 * mu**2 * sigma2**2,
        )  # Var[x^2] of the modes
    ...
    def nlogp(self, x):
        ...
        N1 = (
            (1.0 - self.f)
            * jnp.exp(-0.5 * jnp.sum(jnp.square(x), axis=-1) / self.sigma1**2)
            / jnp.power(2 * jnp.pi * self.sigma1**2, self.d * 0.5)
        )
        N2 = (
            self.f
            * jnp.exp(
                -0.5 * jnp.sum(jnp.square(x - self.mu), axis=-1) / self.sigma2**2
            )
            / jnp.power(2 * jnp.pi * self.sigma2**2, self.d * 0.5)
        )
...
class Funnel_with_Data:
    ...
    def nlogp(self, x, subset):
        ...
        prior_z = np.sum(subset) * theta + jnp.exp(-theta) * jnp.sum(
            jnp.square(z * subset)
        )
...
if __name__ == "__main__":
    rng = np.random.RandomState(seed=10 & (2**32 - 1))
    eigs = rng.gamma(shape=0.5, scale=1.0, size=100)  # eigenvalues of the Hessian
