...
import blackjax
from benchmarks.mcmc.sampling_algorithms import run_mclmc, run_mhmclmc, run_nuts, samplers
from benchmarks.mcmc.inference_models import Brownian, GermanCredit, ItemResponseTheory, MixedLogit, StandardNormal, StochasticVolatility, models
...
def benchmark_mhmchmc(batch_size):
    ...
    for model in models:
        results = defaultdict(tuple)
        for preconditioning, coeffs in itertools.product([False, True], coefficients):
            ...
            for target_acc_rate in [0.65, 0.9]:
                ...
                ess, grad_calls, params , acceptance_rate, _ = benchmark_chains(
                    model, 
                    partial(run_mhmclmc, target_acc_rate=target_acc_rate, coefficients=coeffs, frac_tune1=0.1, frac_tune2=0.1, frac_tune3=0.0, preconditioning=preconditioning), 
                    key1, 
                    n=num_steps, 
                    batch=num_chains, 
                    contract=contract)
                ...
                ess, grad_calls, params , acceptance_rate, _ = benchmark_chains(
                    model, 
                    partial(run_mhmclmc, target_acc_rate=target_acc_rate,coefficients=coeffs, frac_tune1=0.1, frac_tune2=0.1, frac_tune3=0.1, preconditioning=preconditioning), 
                    key1, 
                    n=num_steps, 
                    batch=num_chains, 
                    contract=contract)
...
def run_benchmarks_simple():
    ...
    ess, grad_calls, params , acceptance_rate, step_size_over_da = benchmark_chains(model, partial(sampler, coefficients=coefficients, preconditioning=False),key1, n=num_steps, batch=num_chains, contract=contract)
