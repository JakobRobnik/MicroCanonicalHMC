...
models = {
    Gaussian(d, k, eigenvalues=eigenval_type): {'mclmc': 100000, 'adjusted_mclmc': 40000, 'adjusted_mchmc': 40000, 'nuts': 40000}
    for k in np.ceil(np.logspace(1, 5, num=10)).astype(int)[:] for eigenval_type in ["log", "outliers"] for d in [100]
    }

run_benchmarks(batch_size=128, models=models, key_index=25, do_grid_search=False, do_non_grid_search=False, do_fast_grid_search=True, return_ess_corr=False)