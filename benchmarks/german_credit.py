...
def target_nlog_prob_fn(z):
    ...
    return -(
        target.unnormalized_log_prob(x)
...
class Target:
    def __init__(self):
        ...
        self.grad_nlogp = lambda x: (
            target_nlog_prob_fn(x),
            target_nlog_prob_grad_fn(x),
        )
...
def map_solution():
    ...
    def optimize(z_init, objective_fn, objective_grad_fn, learning_rate, num_steps):
        ...
        return jax.lax.scan(
            lambda z, _: opt_step(z), init=z_init, xs=None, length=num_steps
        )

    z_map, objective_trace = optimize(
        z_init=jnp.zeros(
            target.default_event_space_bijector.inverse_event_shape(target.event_shape)
        ),
...
def ground_truth(key_num):
    ...
    samples, steps, steps_warmup = sample_nuts(
        numpyro_target,
        mchmc_target,
        None,
        10000,
    ...
    np.save(
        "ground_truth/" + name + "/ground_truth_" + str(key_num) + ".npy",
        [second_moments, variance_second_moments],
    )
...
def richard_results():
    ...
    X = np.concatenate(
        [
            ...
            np.swapaxes(lam.T, 1, 2),
            np.swapaxes(beta.T, 1, 2),
        ]
    )
    ...
    bias = np.sqrt(
        np.average(
            np.square(
                ((np.cumsum(np.square(X), axis=2) / np.arange(1, 10001)).T - var) / var
            ),
            axis=2,
        ).T
    )
    ...
    print(
        "ESS = {0}, ESS (with tunning) = {1}".format(
            np.average(ess), np.average(ess_with_tunning)
        )
    )
...
def joint_ground_truth():
    data = np.array(
        [
            np.load("ground_truth/" + name + "/ground_truth_" + str(i) + ".npy")
            for i in range(3)
        ]
    )
