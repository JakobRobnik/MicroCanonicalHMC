...
models = {model(d): {'mclmc': 10000, 'adjusted_mclmc': 5000, 'nuts': 10000}
    for d in second_list for model in [lambda dim: Gaussian(dim, condition_number=1., eigenvalues='linear'), lambda dim : Gaussian(dim, condition_number=1000, eigenvalues='log'), 
                                    #    lambda d: Rosenbrock(d)
                                       ]
    }

run_benchmarks(batch_size=4, models=models, key_index=20, do_grid_search=False, do_non_grid_search=True, do_fast_grid_search=True, return_ess_corr=False, integrators = ["omelyan", "mclachlan", "velocity_verlet"])
